...
Also, the norm of the deviatoric stress satisfies $|\tau|^2 = \tau_{xz}^2 + \tau_{yz}^2 + O(\eps^2)$, the shear stresses are dominated by the $z$ derivatives of the horizontal velocity, that is, $\tau_{xz} = \nu(\frac{\partial u}{\partial z} + \eps^2 \frac{\partial w}{\partial z})$ and $\tau_{yz} = \nu(\frac{\partial v}{\partial z} + \eps^2 \frac{\partial w}{\partial y})$, the stress-free condition at the surface $z=h$ becomes $\tau_{xz} = O(\eps^2)$ and $\tau_{yz} = O(\eps^2)$, and incompressibility \eqref{incompressible} is unaltered by the scaling.
...
\end{equation}
These subject to $\bm{\tau}=0$ at the surface $z=h$ and $\bu=0$ on the base $z=b$.  Note that equations \eqref{siavectorized} essentially use the approximation
...
\section{The weak form of the decoupled Stokes model} \label{sec:weakdecoupled}

In order to implement FE solutions we will need the weak form for the velocity-pressure boundary value problem.  We recall this well-known weak form of the Glen-Stokes problem \eqref{stokes}--\eqref{basebc} \cite{JouvetRappaz2011}; compare the even better-known weak form of the linear Stokes problem \cite{Bueler2021,Elmanetal2014}.  Furthermore this section sets notation and provides a starting point from which to state the coupled model, i.e.~including the surface kinematical equation, in the next section.
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=0.75\textwidth]{figs/currenttime.pdf}
\vspace{-6mm}

\includegraphics[width=0.75\textwidth]{figs/referencedomain.pdf}
\vspace{-1mm}

\includegraphics[width=0.75\textwidth]{figs/nexttime.pdf}
\end{center}
