...
Next we consider the weak form of the Laplace problem \eqref{claplace} for the displacement $c$ on $\Lambda$.  We weakly impose the top boundary condition because it involves the velocity unknowns.  Let $W_0^{1,2}(\Lambda)$ be the space of $W^{1,2}$ scalar functions on $\Lambda$ which are zero on the base $\underline{\partial} \Lambda$.  Let $\cref=-\max\{\Href+b-h^{n-1},0\}$; this data is known in the context of problem \eqref{claplace}.  We multiply $-\grad^2 c=0$ by a test function $e$ and integrate by parts, then use the $\underline{\partial} \Lambda$ and $\partial_{|} \Lambda$ boundary conditions to eliminate all but the top boundary integral, and thereby define a bilinear form:
\begin{equation}
B(c;e) = \int_\Lambda \grad c \cdot \grad e \,dr ds + \int_{\overline{\partial} \Lambda} \left(c - \cref + \Delta t(a + \bu\cdot \left<-\grad_{\xi,\eta} h,1\right>)\right) e\,d\sigma. \label{claplaceweakform}
\end{equation}
