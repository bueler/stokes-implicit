...
Denote the current ice domain by $\Omega^{n-1}=\{\bx\,\big|\,(x,y)\in R, b(x,y)<z<h(t_{n-1},x,y)\} \subset \RR^d$; see \eqref{Omegat}.  The reference domain $\Lambda \subset \RR^d$ is constructed as follows (Figure \ref{fig:domainupdate}):
\begin{equation}
...
\end{equation}
Here the quantities on the right are evaluated at the new time $t_n$ and at the ice surface; as before $a,\bu,h$ denote the surface mass balance, velocity, and elevation, respectively, and specifically $a=a_{\perp}(t_n,x,y,h(t_n,x,y))$.  However, \eqref{backwardeulerske} applies only where ice is present in the solution geometry; we actually seek to approximately solve complementarity conditions \eqref{stronginequality} and \eqref{strongcomplementarity}.
...
\end{align}
Figure \ref{fig:claplaceproblem} sketches this problem.  Noting that the solution $c$ will be used to vertically perturb the domain ($z=\zeta+c(\xi,\eta,\zeta)$), the basal and side boundary conditions are clear; the latter condition makes the application of a side boundary condition at some finite distance relatively unobtrusive.

\begin{figure}[h]
\begin{center}
\includegraphics[width=0.85\textwidth]{figs/claplaceproblem.pdf}
\end{center}
\caption{A sketch of domain $\Lambda$ and problem \eqref{claplace}.  See the text regarding the shaded ``miasma'' volume (equation \eqref{miasma}) and the ``$\ast$'' notation.}
\label{fig:claplaceproblem}
\end{figure}

However, the top-surface Dirichlet value, a surface displacement which will be included weakly into the coupled system (section \ref{sec:finiteelement} below), needs comment.  Where the current geometry is more than $\Href$ above the bed the value is simply the (discretized) surface kinematical quantity \eqref{backwardeulerske}.  On the other hand, in currently ice-free areas, which satisfy $h^{n-1}=b$ and $\bu=\bzero$, as marked by ``$\ast$'' in Figure \ref{fig:claplaceproblem} the value is
\begin{equation}
    c = - \Href + \Delta t\,a. \label{icefreehrefske}
\end{equation}
In fact, assuming $h^{n-1}$ is itself continuous, as one passes across the glacier margin the top surface value in problem \eqref{claplace} changes continuously from the one in \eqref{backwardeulerske} to that in \eqref{icefreehrefske}.

The shaded area of the Figure, the portion of the reference domain not occupied by ice ($\Lambda \setminus \Omega^{n-1}$) is, properly-speaking, the volume of the atmosphere where the ice currently extends less than $\Href$ above the bed.  Note that on the top of this volume the ``$\max$'' in \eqref{claplace} adds nontrivially to the value from \eqref{backwardeulerske}.  Here our model will include an incompressible regularizing fluid which we call ``miasma.''\footnote{Defined as an oppressive or unpleasant atmosphere which surrounds something, often settling into low areas.}  For miasma the density is reduced to a smaller reference value and the fluid becomes linear with a small viscosity.  Specifically, \eqref{forcebalance} and \eqref{viscflowlaw} are replaced by
\begin{equation} % FIXME: proposed flow law
- \nabla \cdot \tau + \nabla p = \rhom \bg, \qquad \tau = 2 \num D\bu \label{miasma}
\end{equation}
respectively, and Equation \eqref{incompressible} is unchanged.  In our numerical experiments (section \ref{sec:results} below) we will use a density which is a tenth that of ice and a viscosity which is a tenth of that of ice deforming at the reference strain rate $D_0$ (equation \eqref{regeffvisc}): $\rhom = \frac{1}{10} \rho$, $2\num = \frac{1}{10} B_n D_0^{1/n}$.  The intention is that such a weak and light fluid will have minimal impact on the dynamics of the ice while also occupying a part of the domain which the ice may come to occupy during the solution process for the coupled equations.
...
\end{equation}
with determinant $\det(J)=1+\frac{\partial c}{\partial \zeta}$.  For a generic, smooth function $f(\bx)$ defined on $\Omega^n$ we may define a function on $\Lambda$, namely $\tilde f(\bxi) = f(\bx(\bxi))$, whose gradient transforms as $\grad_{\bxi} \tilde f = J^\top \grad_{\bx} f$.  Thus
\begin{equation}
...
where
\begin{equation}
\ell(\bxi) = \frac{1}{1+\frac{\partial c}{\partial \zeta}}. \label{definel}
\end{equation}
and
\begin{equation}
j(\bxi) = - \frac{\partial c}{\partial \xi}\, \ell(\bxi), \quad k(\bxi) = -\frac{\partial c}{\partial \eta}\, \ell(\bxi). \label{definejk}
\end{equation}

FIXME $\frac{\partial c}{\partial \zeta}$ is a dimensionless number; in the absence of surface balance ($a=0$) we expect it to be roughly $-1$ in currently ice free regions of $\Lambda$ (cause for $c(\zeta)$ as a two-point boundary value problem, the equations $c''=0$, $c(\Href)=-\Href$, $c(0)=0$ give $c(\zeta)=-\zeta$)

\newcommand{\deltaref}{\delta_{\text{ref}}}
FIXME: NOT THIS regularize top boundary condition for $c$ so that the denominator of the formulas for $j,k,\ell$ is bounded away from zero:
