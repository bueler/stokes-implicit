...
\end{align}
Figure \ref{fig:claplaceproblem} sketches this problem.  Noting that the solution $c$ will be used to vertically perturb the domain ($z=\zeta+c(\xi,\eta,\zeta)$), the basal boundary condition means that the map \eqref{changecoords} sends the base of $\Lambda$ to the base of $\Omega^n$.  The Neumann side boundary condition, applied at some finite distance from the current ice, is relatively unobtrusive.  Intuitively, $c$ is a smooth function which, in each vertical column of $\Lambda$, connects the surface kinematical quantity \eqref{backwardeulerske} as linearly as possible to zero at the base.  Mathematically, $c$ satisfies the maximum principle in $\Lambda$ \cite{Evans2010}, and the direction of $\grad_{\bxi} c$ changes direction as little as possible (in the energy norm) while smoothly meeting the boundary data.
...
FIXME \eqref{claplace} is a smoother form of the independent-column two-point boundary value problem $c''=0$, $c(h)=X$, $c(b)=0$ on $\zeta\in[h,b]$ which gives $c(\zeta)=(X-b)\zeta/(h-b)$; essentially this is the vertical ``linear mapping'' in \cite[equation (22)]{Lengetal2012}
