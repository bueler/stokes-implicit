...
Next we note the key fact, which is an admissibility requirement for the function $h(x,y,t)$, as a part of the model solution, that
\begin{equation}
...
Our assumption that the top of the icy region is described by a well-defined surface elevation function $h(x,y,t)$ represents a kind of ``shallowness'' assumption.  In agreement with almost all ice sheet and glacier modeling literature, we assume that for each map-plane location $(x,y)$ and time $t$,
\begin{equation}
I(x,y,t) = \{z\,\big|\,\bx=(x,y,z) \in \Omega^t\} \label{intervalassume}
\end{equation}
is a single (open) real interval, and in particular that there are no ice overhangs.  Note that $I(x,y,t)$ is the empty set when ice is not present at map-plane location $(x,y)$.
