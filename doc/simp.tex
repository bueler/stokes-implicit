...
Suppose $h^{n-1}(x,y) \approx h(t_{n-1},x,y)$ is our current surface elevation.  Denote the corresponding ice domain by $\Omega^{n-1}=\{\bx\,\big|\,(x,y)\in R, b(x,y)<z<h^{n-1}(x,y)\} \subset \RR^3$; see \eqref{Omegat} and the top part of Figure \ref{fig:domainupdate}.  The reference domain $\Lambda \subset \RR^3$ will be constructed so that its thickness is always at least $\Href$, so for $(x,y) \in R$ let
\begin{equation}
\lambda(x,y) = b(x,y) + \max\{\Href,h^{n-1}(x,y)-b(x,y)\}. \label{definelambda}
\end{equation}
Observe that $\lambda$ is given by data which is known at the start of the time step.  Now define the reference domain to have surface $z=\lambda(x,y)$:
\begin{equation}
\Lambda = \left\{(\xi,\eta,\zeta)\,\big|\,(\xi,\eta)\in R, \, b(\xi,\eta) < \zeta < \lambda(\xi,\eta)\right\};  \label{Lambda}
\end{equation}
see the middle part of Figure \ref{fig:domainupdate}.  Observe that $\Omega^{n-1} \subseteq \Lambda$ and $h^{n-1} \le \lambda$.

From now on $\bxi=(\xi,\eta,\zeta)$ will denote coordinates on $\Lambda$.  We will compute the updated domain $\Omega^n \subset \RR^3$ via a change of coordinates map $\bxi \mapsto \bx$,
\begin{equation}
\bx(\bxi) = \bxi + (0,0,c(\bxi)), \label{changecoords}
\end{equation}
based on a new scalar function $c(\bxi)$ which is defined on $\Lambda$ and satisfies certain properties below.  One of these is that
\begin{equation}
...
\end{equation}
Determining $c(\bxi)$ on $\Lambda$ will therefore determine the new ice domain $\Omega^n$.  Since $c(\bxi)$ will solve an elliptic PDE (below), the map is smooth from $\Lambda$ to $\Omega^n$, but it is generally degenerate.  It is not a diffeomorphism because its inverse is not generally smooth or even well-defined.  In particular, applying the topological interior in \eqref{updateddomain} is important because the map will take ice free locations in $\Lambda$ to locations which have zero height and are outside of $\Omega^n$.

We now assume that the map $\bxi \mapsto \bx$ in \eqref{changecoords} is monotonic in each column where ice is present in the solution.  Consider the computed surface elevation $h(\xi,\eta)\approx h(t_n,\xi,\eta)$, which includes the upper boundary of $\Omega^n$ but which is defined on all of $R$.  Since the base is fixed $z(\xi,\eta,b(\xi,\eta))=b(\xi,\eta)$, the ice is above the bed $z(\xi,\eta,\zeta) \ge b(\xi,\zeta)$, and the map is differentiable this means that
\begin{equation}
1 + \frac{\partial c}{\partial \zeta}(\bxi) = \frac{\partial z}{\partial \zeta}(\bxi) \ge 0. \label{mapmonotonic}
\end{equation}
It follows by maximizing $\zeta$ in each column that $h(\xi,\eta)$ satisfies
\begin{equation}
h(\xi,\eta) = \lambda(\xi,\eta) + c(\xi,\eta,\lambda(\xi,\eta)) \label{maptoptotop}
\end{equation}
at all locations $(\xi,\eta)\in R$.  That is, the top of the reference domain is sent to the top of the new ice domain.  In locations which are computed to be ice free we have
\begin{equation}
