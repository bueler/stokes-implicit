...
\end{equation}
At any given map-plane location $(x,y)$ the ice may be present (strict inequality in \eqref{admissibility}) or absent (equality) in an evolving, time-dependent manner.  As also shown in Figure \ref{fig:domainnotation}, the time-dependent ice-covered map-plane domain, which is the vertical projection of $\Omega^t$, is denoted $\pi \Omega^t$.
...
In agreement with almost all ice sheet and glacier modeling literature, \eqref{Omegat} states that both the upper and lower surfaces of the ice mass are described by a well-defined functions of map-plane location.  The domain $\Omega^t$ is thus a layer \cite{Bueler2020}, and not general.  Furthermore, in the usual case where $\partial_{\text{in}} \Omega^t=\emptyset$, we assume a disjoint decomposition $\partial \Omega^t = \overline{\partial} \Omega^t \cup \underline{\partial} \Omega^t$ except for a set of measure zero along the margin.  Looking ahead, this layer assumption is compatible with our FE method based on a vertically-extruded, unstructured mesh in the map plane.
...
\end{equation}
where $\bn = \left<-\frac{\partial h}{\partial x},-\frac{\partial h}{\partial y},1\right>$ points upward and is normal to the ice surface.  The surface mass balance $a$ in \eqref{surfacekinematical} is the trace \cite{Evans2010} of the mass balance along the ice surface, i.e.~$a = a_{\perp}(t,x,y,h(t,x,y))$.  Modeled melting or freeze-on at the ice base would add a basal kinematic equation \cite[for example]{Aschwandenetal2012}, but for simplicity this is not considered here.

Equation \eqref{surfacekinematical} applies on the ice surface, that is, where ice is present.  However, the equation should be understood as the following statements which hold on the entire computational domain $R$ \cite{Bueler2016,Bueler2020,SchoofHewitt2013}:
\begin{align}
...
\end{align}
along with the fundamental inequality \eqref{admissibility}.  At ice-free locations, where $\frac{\partial h}{\partial t}=0$ and $\bu=\bzero$, \eqref{stronginequality} says that $a \le 0$, that is, the surface mass balance is not positive.  Physically-speaking, snow melt and runoff exceed solid precipitation, and ice is not flowing into, such an ice free location.  Equation \eqref{strongcomplementarity}, a complementarity principle, says that either there is no ice at the location ($h=b$) or ice is present and equation \eqref{surfacekinematical} applies.  Thus \eqref{admissibility}, \eqref{stronginequality}, and \eqref{strongcomplementarity} together, which form a nonlinear complementarity problem \cite{Bueler2016,Bueler2020}, imply \eqref{surfacekinematical} on the ice surface and $a\le 0$ at ice-free locations.  They do so without referring to glacier extent, which we regard as part of the solution.

Note that the mass balance $a_{\perp}(t,x,y,z)$ is computed using a separate model for the dynamical state of the atmosphere, one which computes snow precipitation, (atmospheric) energy-driven melt at snow/ice surfaces, and liquid water runoff.  How this is done is well beyond our scope.  However, it is important in moving-margin glacier simulations that $a$ have a well-defined value both at the current ice surface and elsewhere, or at least nearby, because a simulated glacier can only move into new areas, retreat from ice-covered areas, and undergo changes of surface elevation, if the source term at the new (and intermediate) locations is well-defined.

Informally, \eqref{surfacekinematical} describes how the ice surface elevation is updated: the climatically added or removed ice $a\,\Delta t$, plus the component of the ice motion in direction $\bn$, determines the vertical displacement of the surface.  Numerical glacier models therefore often evolve the time-dependent surface $z=h$ by explicit steps, i.e.~$\Delta h \approx \left(a + \bu\cdot \bn\right) \Delta t$ in the simplest forward-Euler case, based upon the current values of $a$, $\bu$, and $\bn$ (i.e.~$\grad h$).  However, we will be solving \eqref{surfacekinematical}, more precisely \eqref{admissibility}, \eqref{stronginequality} and \eqref{strongcomplementarity}, implicitly and coupled with the Glen-Stokes system \eqref{stokes}--\eqref{basebc}.  The new ice surface will be compatible with the flow model when the solver has converged.  Appendix A summarizes a simplified model of the same coupled problem, namely the shallow ice approximation (SIA), but we will only need it as a source of initial conditions and in performance comparisons.


\section{Implicit time-stepping and the domain update} \label{sec:implicitstep}

Our main purpose is to demonstrate an implicit scheme to simultaneously solve the Glen-Stokes problem \eqref{stokes}--\eqref{basebc} and the surface kinematical equation \eqref{surfacekinematical}.  In this section we state this scheme using a discrete-time and continuous-space formulation.  A new feature is that at each time step we compute a surrogate for the ice strain.  This vertical-only displacement, defined everywhere within a reference domain, is computed simultaneously with the velocity and pressure, and, once found, it determines the new domain of ice.

The only time derivative in our coupled problem appears in \eqref{surfacekinematical}.  Because the remaining equations and boundary conditions apply at all instants, the coupled problem should be understood as an infinite-dimensional differential algebraic equation problem \cite{AscherPetzold1998}.  We discretize the problem using first-order, backward Euler time-stepping.  Higher-order implicit schemes exist, including stiff-decay backward-differentiation schemes \cite{AscherPetzold1998,Bueler2021}.  However, because the coupled solution is also subject to inequality constraints \eqref{admissibility} and \eqref{stronginequality}, this free-boundary problem has the character of a time-dependent variational inequality \cite{Calvoetal2002} or nonlinear complementarity \cite{Bueler2016} problem.  The numerical errors associated to determining the free boundary are unavoidably of low order in time and space \cite{Bueler2020} and thus they tend to dominate over time-stepping errors made in well-behaved locations.

The fundamental issue faced by implicit time-stepping schemes for glaciers is that the map-plane region covered by ice changes during the time step.  Either in terms of the 3D continuum domain of the coupled PDE problem, or in terms of the 3D mesh used for numerical simulations, one can only solve for ice velocity and pressure in locations where ice or mesh, respectively, exists.  Concretely, supposing $t_{n-1}$ and $t_n$ are consecutive times with step $\Delta t = t_n - t_{n-1} > 0$, the possibility of glacier advance and retreat during an implicit step therefore requires the PDE domain to cover the current ice (time $t_{n-1}$), \emph{and} we must somehow extend the domain to locations where ice will be present once we have solved the equations for the new time ($t_n$).
...
Our scheme uses the current ice geometry to construct a reference domain on which the new velocity, pressure, and vertical displacement are defined once the coupled problem is solved.  The icy domain is allowed to change, including changes in the map-plane topology as the glacier disconnects, for example.
...
Suppose $h^{n-1}(x,y)$ is our current surface elevation.  Denote the current ice domain by $\Omega^{n-1}=\{\bx\,\big|\,(x,y)\in R, b(x,y)<z<h^{n-1}(x,y)\} \subset \RR^d$; see \eqref{Omegat}.   For $(x,y) \in R$ let
\begin{equation}
\lambda(x,y) = b(x,y) + \max\{\Href,h^{n-1}(x,y)-b(x,y)\}. \label{definelambda}
\end{equation}
Now, the reference domain $\Lambda \subset \RR^d$ is constructed so that its thickness is always at least $\Href$:
\begin{equation}
\Lambda = \left\{(\xi,\eta,\zeta)\,\big|\,(\xi,\eta)\in R, \, b(\xi,\eta) < \zeta < \lambda(\xi,\eta)\right\};  \label{Lambda}
\end{equation}
see Figure \ref{fig:domainupdate}.  Note that $\zeta=\lambda(\xi,\eta)$ gives the elevation of the top boundary of $\Lambda$.  Observe that $\Omega^{n-1} \subseteq \Lambda$ and $h^{n-1} \le \lambda$; the latter inequality is strict in currently (time $t^{n-1}$) ice free areas.
...
\end{equation}
based on a new scalar function $c(\bxi)$ which is defined on $\Lambda$ and satisfies certain properties which we must identify.  Note that the horizontal coordinate is unaltered, but the $z$-coordinate $z=\zeta+c(\xi,\eta,\zeta)$ is nontrivial.  The domain $\Omega^n$ is defined to be the interior of the image of $\Lambda$ under map \eqref{changecoords}:
\begin{equation}
\Omega^n = \{\bx(\bxi) \,\big|\, \bxi \in \Lambda\}^\circ. \label{updateddomain}
\end{equation}
Since $c(\bxi)$ will solve an elliptic PDE (below), the new coordinates $\bx$ are smooth functions of $\bxi$ on $\Lambda$, but the map is generally degenerate; it is not a diffeomorphism because its inverse is not generally smooth or even well-defined.  In particular, applying the topological interior in \eqref{updateddomain} is important because the map will take ice free locations in $\Lambda$ to locations which have zero height and are outside of $\Omega^n$.

It follows from \eqref{changecoords} and \eqref{updateddomain}, by maximizing $\zeta$ in each column within $\Lambda$, that the computed surface elevation satisfies
\begin{equation}
h(\xi,\eta) = \lambda(\xi,\eta) + c(\xi,\eta,\lambda(\xi,\eta)) \label{hmeaning}
\end{equation}
at all locations $(\xi,\eta)\in R$.  Given that $\lambda$ in \eqref{definelambda} is given by data which is known at the start of the time step, a computation of $c(\bxi)$ on $\Lambda$ will determine the new ice surface elevation.  Note that both the current ($h^{n-1}$) and computed ($h$) surface elevations are defined on all of $R$.

Now that we have defined the reference domain we return to surface kinematical equation \eqref{surfacekinematical} and approximate it with a backward Euler step.  That is, where the computed surface elevation $h(\xi,\eta) \approx h(t_n,\xi,\eta)$ indicates that we have ice, i.e.~$h>b$, we have the following equation:
\begin{equation}
...
So, what properties, other than smoothness, must $c(\bxi)$ have so that the above definitions and statements \eqref{definelambda}--\eqref{backwardeulerske} are all consistent?  FIXME DEDUCE CONSEQUENCES WHERE THE SOLUTION HAS ICE ($h>b$) AND WHERE ICE FREE ($h=b$)
